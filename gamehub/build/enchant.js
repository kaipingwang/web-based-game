...
enchant.Timeline = enchant.Class.create(enchant.EventTarget, {
 ...
     * You can choose time based and frame based(default) animation.
    ...
    clear: function() {
        ...
        this.queue = [];
        this._deactivateTimeline();
        document.getElementById("showResult").innerHTML = "Congratulation You Made It ";
    ...
    pause: function() {
        if (!this.paused) {
            ...
            this._deactivateTimeline();
        $("#instruction").remove();
        document.getElementById("showResult").innerHTML = "Congratulation You Did It ";
    ...
    removeFromScene: function() {
        return this.then(function() {
            ...
            if(this==alien){
              $("#instruction").remove();
              localStorage.answerA="show(bossA);show(bossB)"+ '\n'+
               "moveRight(20);shooting()"+ '\n'+
               "moveLeft(40);shooting()"+ '\n'+
               "moveY(prizeA);moveX(prizeB)"
               localStorage.hintA="Try integrating all the method we used\
                                   before such as show,shooting,moveLeft,\
                                   moveTo"
               $('#solution').on('click', function() {
                 swal({
                 title: localStorage.answerA,
                 imageUrl: "Hint.gif",
                 imageSize:"300x180"
               });
               })
               $('#hint').on('click', function() {
                 swal({
                 title: localStorage.hintA,
                 imageUrl: "Hint.gif",
                 imageSize:"300x180"
               });
               })
               document.getElementById("showResult").innerHTML = "Find Invisible BossA and BossB, then Shooting them and Collecting Treasure";

            }else if (this==bomb){
              $("#instruction").remove();
              var answerB ="show(alien)"+'\n'+
                           "removeFromScene(alien)"
                var hintB ="Try using the show mehod"
                            $('#solution').on('click', function() {
                              swal({
                              title: answerB,
                              imageUrl: "Hint.gif",
               ...
               document.getElementById("showResult").innerHTML = "Find Invisible Alien and Remove It ";
            }
        });
    },
    /**
